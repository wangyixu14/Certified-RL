...
def constraintsAutoGenerate():
	...
	## Lyapunov function variable declaration
	X = MatrixSymbol('X', 6, 6)
	Y = MatrixSymbol('Y', 10, 10)
	M = MatrixSymbol('M', 3, 3)
	N = MatrixSymbol('N', 3, 3)

	P = MatrixSymbol('P', 1, 14) # Laypunov
	a = MatrixSymbol('a', 1, 6) # sigma 1
	b = MatrixSymbol('b', 1, 6) # sigma 2
	theta = MatrixSymbol('t', 1, 2)

	s, v, l = symbols('s, v, l')
	ele = Matrix([1, s, v, s**2, s*v, v**2]) 
	base = Matrix([1, s, v]) # for sigma 1 and 2
	Lya_ele = Matrix([s, v, s**2, s*v, v**2, s**3, s**2*v, s*v**2, v**3, s**4, s**3*v, s**2*v**2, s*v**3, v**4])

	rhsX = ele.T*X*ele
	rhsX = expand(rhsX[0, 0])
	lhsX = P*Lya_ele - a*ele*(1 - s**2 - v**2)
	lhsX = expand(lhsX[0, 0])
	generateConstraints(rhsX, lhsX, degree=4)
	...
	rshY = Matrix([1, s, v, s**2, s*v, v**2, s**3, s**2*v, s*v**2, v**3]).T*Y*Matrix([1, s, v, s**2, s*v, v**2, s**3, s**2*v, s*v**2, v**3])
	rshY = expand(rshY[0, 0])
	Lyapunov = P*Lya_ele
	partials = diff(Lyapunov[0, 0], s)
	partialv = diff(Lyapunov[0, 0], v)
	# print(expand(partials.doit()))
	# print(partials, partialv)
	# assert False
	gradVtox = Matrix([[partials, partialv]])
	controlInput = theta*Matrix([[s], [v]])
	controlInput = expand(controlInput[0,0])
	dynamics = Matrix([[v], [(s**3) / 3 + controlInput]])
	lhsY = -gradVtox*dynamics - b*ele*(1 - s**2 - v**2) 
	lhsY = expand(lhsY[0, 0])
	generateConstraints(rshY, lhsY, degree=6)
	# print(rshY)
	# print('')
	# print(lhsY)
	# assert False

	b_SOS_right = base.T*N*base
	b_SOS_right = expand(b_SOS_right[0, 0])
	b_SOS_left = b*ele
	b_SOS_left = expand(b_SOS_left[0, 0])
	SOSConstraints(b_SOS_right, b_SOS_left)
